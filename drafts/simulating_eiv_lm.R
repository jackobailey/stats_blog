...
for(sim in which(is.na(sim.out))) {
  ...
  x1.mod <- mirt(data = data.frame(x1a,x1b, x1c, x1d, x1e, x1f, x1g, x1h),
                 model = 1, verbose = FALSE)
  x2.mod <- mirt(data = data.frame(x2a,x2b, x2c), 
                 model = 1, verbose = FALSE)
...
sim.summary <- aggregate(sim.all[, c("in95ci", "resid", "abs.resid", "coefficient", 
                                     ...
                                     "resid2")], sim.all[, c("var", "error", "estimator", "r.method")], 
          FUN = mean)
...
library(dplyr)
sim.summary %>% arrange(estimator, desc(error), r.method, var)

save(sim.summary, file = "resources/irt_eiv_sims100.rda")
