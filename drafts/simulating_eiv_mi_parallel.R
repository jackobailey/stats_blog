...
simMIEIV <- function(sim, include.pv = TRUE, 
                     alpha = 1,
                     beta_1 = 1,
                     beta_2 = 1,
                     n = 2000,
  ...
  x2d <- rbinom(prob = plogis(x2 + rnorm(n, sd = 5)), size = 1, n=n)
  
  x1dat <- data.frame(x1a,x1b, x1c, x1d, x1e, x1f, x1g, x1h)
  x2dat <- data.frame(x2a,x2b, x2c, x2d)
  ...
  data.classical2$rxx1 <- cor(cbind(data.classical$x1, x1))[1,2]^2
  data.classical2$rxx2 <- cor(cbind(data.classical$x2, x2))[1,2]^2
  ...
  if(include.pv) {
    ...
    x1.mod.ycov <- mirt(data = x1dat,
                   model = 1, verbose = FALSE, 
                   covdata = data.frame(y), 
                   formula = ~.)
    x2.mod.ycov <- mirt(data = x2dat, 
                   model = 1, verbose = FALSE,
                   covdata = data.frame(y), 
                   formula = ~.)
    pv_x1.ycov <- fscores(x1.mod.ycov,  plausible.draws = 50)
    pv_x2.ycov <- fscores(x2.mod.ycov,  plausible.draws = 50)
    
    
    draw_merger_ycov2 <- function(draw_num = 1){
      sim_data_draw <- sim_data %>% 
        mutate(x1 = pv_x1.ycov[[draw_num]][,1] / sd(pv_x1.ycov[[draw_num]][,1]), 
               x2 = pv_x2.ycov[[draw_num]][,1] / sd(pv_x2.ycov[[draw_num]][,1]))
      return(sim_data_draw)
    }
    pv_imputes_ycov2 <- map(1:length(pv_x1.ycov), draw_merger_ycov2) %>%
      imputationList()
    mi_mods_ycov2 <- with(pv_imputes_ycov2,  lm(y ~ x1 + x2))
    pv.mod.ycov2 <- MIcombine(mi_mods_ycov2)
    
    pv.temp.out <- data.frame(var = names(lm.mod.temp$coefficients),
                              coefficient = pv.mod.ycov2$coefficients, 
                              se = diag(sqrt(pv.mod.ycov2$variance)), 
                              error = "PV: covariates", 
                              estimator = "MI", 
                              r.method = "None",
                              reliability = NA)
    pv.temp.out$true.cor.latent <- true.cors[pv.temp.out$var]
    temp.sim.res[["pvycov2"]] <- pv.temp.out
  }
  ...
  if(include.pv) {
    ...
    pv_x1.simple <- fscores(x1.mod,  plausible.draws = 50)
    pv_x2.simple <- fscores(x2.mod,  plausible.draws = 50)
    
    
    draw_merger_simple <- function(draw_num = 1){
      sim_data_draw <- sim_data %>% 
        mutate(x1 = pv_x1.simple[[draw_num]][,1] / sd(pv_x1.simple[[draw_num]][,1]), 
               x2 = pv_x2.simple[[draw_num]][,1] / sd(pv_x2.simple[[draw_num]][,1]))
      return(sim_data_draw)
    }
    pv_imputes_simple <- map(1:length(pv_x1.simple), draw_merger_simple) %>%
      imputationList()
    mi_mods_simple <- with(pv_imputes_simple,  lm(y ~ x1 + x2))
    pv.mod.simple <- MIcombine(mi_mods_simple)
    
    pv.temp.out <- data.frame(var = names(lm.mod.temp$coefficients),
                              coefficient = pv.mod.simple$coefficients, 
                              se = diag(sqrt(pv.mod.simple$variance)), 
                              error = "PV: simple", 
                              estimator = "MI", 
                              r.method = "None",
                              reliability = NA)
    pv.temp.out$true.cor.latent <- true.cors[pv.temp.out$var]
    temp.sim.res[["pvycov"]] <- pv.temp.out
  }
...
n.sims <- 100
sims.per.round <- 10
sim.sets <- as.list(rep(NA, n.sims / sims.per.round))

for(kk in which(is.na(sim.sets))) {
  simout <- future_lapply(((kk-1)*sims.per.round) + (1:sims.per.round), simMIEIV, 
                          alpha = 1,
                          beta_1 = 1,
                          beta_2 = 1,
                          n = 2000,
...
sim.sets2 <- lapply(sim.sets[!is.na(sim.sets)], function(x) do.call(rbind, x))

sim.all <- do.call(rbind, sim.sets2)
...
sim.summary <- aggregate(sim.all[, c("in95ci", "resid", "abs.resid", "coefficient", 
                                     ...
                                     "resid2", "ci.width", "rx1x2")], sim.all[, c("var", "error", "estimator", "r.method")], 
                         FUN = mean)
...
sim.summary$n.sims <- sum(!is.na(sim.sets)) * sims.per.round
library(dplyr)
sim.summary <- sim.summary %>% arrange(estimator, desc(error), r.method, var)
sim.summary <- sim.summary[(grepl("irt\\.cool|PV:|classical$", sim.summary$error)  & 
                             sim.summary$estimator %in% c("EIV", "MI")) |
                             (sim.summary$error=="irt" & sim.summary$estimator=="OLS") , ]

sim.summary %>% 
  mutate_if(is.numeric, round, 5) %>% tibble()


save(sim.all, file = "resources/irt_eiv_sims100.rda")
